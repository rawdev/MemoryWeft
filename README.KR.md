...
**Memory Weft는 공유 메모리입니다. 여러 AI가 하나의 메모리에 동시 접근이 가능합니다.**

**노트북과 데스크탑에서 따로 돌아가는 AI가, 팀 멤버들이 따로 사용하는 AI가 하나의 메모리에서 컨텍스트를 공유 합니다.**

**AI와의 중요한 대화를 명령어를 사용해 저장하던지 문서를 한번에 변환시켜 메모리에 저장할 수 있습니다.**

**저장된 데이터는 언제든지 명령어를 사용해 불러올 수 있으며, AI는 문맥에 따라 연관된 내용을 추가로 검색해서 결과를 보여줄 것입니다.**

"mw 저장", "mw 확인" 또는 "문서를 모두 mw에 저장해줘" 같은 명령어만 사용하면 됩니다.
...
공유 메모리이기 때문에 한번 추가된 이벤트는 가능하면 삭제하지 않아야 합니다.
...
## Manager
MCP 설치 및 DB 전환을 도와주는 Manager 앱이 있어서 내가 사용할 DB를 쉽게 선택할 수 있습니다.
