"""Source Lineage — Postgres DDL (k2g_raw_document / k2g_segment_blob / purge_audit).

Column order/names match the SQLite schema; only dialect differences:
- JSONB (e.g. source_locator on events)
- TIMESTAMPTZ
- BIGSERIAL
"""
...
CREATE TABLE IF NOT EXISTS k2g_segment_blob (
    id              TEXT PRIMARY KEY,
    raw_document_id TEXT REFERENCES k2g_raw_document(id) ON DELETE SET NULL,
...
def setup_source_lineage_schema_postgres(cur) -> None:
    """Create the 3 source-lineage tables + indexes on the given Postgres cursor."""
...
__all__ = ["setup_source_lineage_schema_postgres"]
