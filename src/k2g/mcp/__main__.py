...
def _require_mwf_file() -> Path | None:
    ...
    env (``K2G_DOTENV_FILE=""`` + ``DATA_DIR`` + ``EMBEDDING_*``), so
    they intentionally skip ``.mwf``. When ``K2G_DOTENV_FILE`` is
    *explicitly empty* (env set to ``""``), disable the safety check and
    return ``None``.

    Legacy: if only ``.chronoGraph`` exists, auto-rename once.
    """
    # Env-level opt-out: user explicitly signals ".mwf not used".
    # Sentinel distinguishes explicit empty from simply missing.
    _SENTINEL = object()
    dotenv_override = os.environ.get("K2G_DOTENV_FILE", _SENTINEL)
    if dotenv_override is not _SENTINEL and (
        dotenv_override == "" or dotenv_override is None  # type: ignore[comparison-overlap]
    ):
    ...
    if not cfg.exists():
        ...
        sys.stderr.write(
            ...
            f" mweft-init\n"
            f"          (set K2G_DOTENV_FILE=\"\" to opt out"
            f" of .mwf).\n"
        )
        raise SystemExit(2)
    return cfg
